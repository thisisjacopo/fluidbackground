...
function init() {
  ...
  GPU.setUniformForProgram(
    "packToBytes",
    "u_floatTextureDim",
    [particlesTextureDim, particlesTextureDim],
    "2f"
  );
  ...
  GPU.initTextureFromData(
    "outputParticleBytes",
    particlesTextureDim * vectorLength,
    particlesTextureDim,
    "UNSIGNED_BYTE",
    null
  ); //2 comp vector [x,y]
...
function setThree() {
  for (var i = 0; i < numParticles; i++) {
    var vertex = new THREE.Vector3(
      Math.random() * actualWidth,
      Math.random() * actualHeight,
      0
    );
  ...
  if (!paused) {
    ...
    if (!mouseout && mouseEnable) {
      ...
      GPU.setUniformForProgram(
        "force",
        "u_mouseCoord",
        [mouseCoordinates[0] * scale, mouseCoordinates[1] * scale],
        "2f"
      );
      GPU.setUniformForProgram(
        "force",
        "u_mouseDir",
        [
          3 * (mouseCoordinates[0] - lastMouseCoordinates[0]) * scale,
          3 * (mouseCoordinates[1] - lastMouseCoordinates[1]) * scale,
        ],
        "2f"
      );
    } else {
    ...
    if (!mouseout && mouseEnable) {
      GPU.setUniformForProgram("addMaterial", "u_mouseEnable", 1, "1f");
      GPU.setUniformForProgram(
        "addMaterial",
        "u_mouseCoord",
        mouseCoordinates,
      ...
      GPU.setUniformForProgram(
        ...
        "u_mouseLength",
        Math.sqrt(
          Math.pow(3 * (mouseCoordinates[0] - lastMouseCoordinates[0]), 2) +
